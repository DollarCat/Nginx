...
void CSocekt::ngx_event_accept(lpngx_connection_t oldc)
{
    ...
    {     
        if(use_accept4)
        {
            //以为listen套接字是非阻塞的，所以即便已完成连接队列为空，accept4()也不会卡在这里；20240520我的理解：设置为非阻塞模式，后边调用recv函数会立即返回不阻塞流程
        ...
        if(!use_accept4)
        {
            //如果不是用accept4()取得的socket，那么就要设置为非阻塞【因为用accept4()的已经被accept4()设置为非阻塞了】,20240520我的理解：设置为非阻塞模式，后边调用recv函数会立即返回不阻塞流程
